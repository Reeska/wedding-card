...
<script lang="ts">
  ...
  export default class Konami extends Vue implements OnMounted {
    private readonly allowedKeys: { [key: number]: string } = {
      37: 'left',
      38: 'up',
      39: 'right',
      40: 'down',
    ...
    private readonly konamiCode = ['up', 'up', 'down', 'down', 'left', 'right', 'left', 'right', 'b', 'a'];
    ...
    public onKeyDown(e: any) {
      ...
      const key: string = this.allowedKeys[e.keyCode];
...
<style lang="scss">
  #ship {
    ...
    top: 20%;
    right: -500px;
    animation: flyshipfly 7s cubic-bezier(0.03, 0.34, 1, 0.05) alternate;
    opacity: 0;
    z-index: 3000;
  }
  ...
  @keyframes flyshipfly {
    from {
      transform: rotate(4deg) scale(0.001);
      opacity: 1;
    }
    to {
      transform: rotate(0deg) scale(1) translate(-500px);
      opacity: 1;
    }
  }
</style>
